...
public interface DemoAuditsMapper {
	...
	@Select(value = "{ CALL SP_GA_GETAUDITSTATUS_DET_PV4( #{guid, mode=IN, jdbcType=BINARY}, #{uid, mode=IN, jdbcType=BINARY},#{client_id, mode=IN, jdbcType=VARCHAR},"
			...
			+ "#{search_item, mode=IN, jdbcType=VARCHAR},#{sort, mode=IN, jdbcType=VARCHAR},#{order, mode=IN, jdbcType=VARCHAR},#{submitted_by, mode=IN, jdbcType=VARCHAR},#{min_score, mode=IN, jdbcType=INTEGER},#{max_score, mode=IN, jdbcType=INTEGER} ) }")
	@Options(statementType = StatementType.CALLABLE)
